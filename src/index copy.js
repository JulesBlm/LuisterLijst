...
- Remove button for bars > remove from order and from youtubes
- Add youtube box
- Add api's methods to Bar component
...
class App extends React.Component {
  ...
  handleMouseMove = ({ pageY }) => {
    ...
    if (isPressed) {
      ...
      console.log({originalPosOfLastPressed});
            ...
            <Spring immediate={name => active && name === 'y'} to={style} key={i}>
              {({ scale, shadow, y }) => (
                <div
                  ...
                  <Player youtubeid={this.state.links[i].youtube} number={i}/>
                </div>
              )}
            </Spring>
          )
